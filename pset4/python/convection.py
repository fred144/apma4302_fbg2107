...
solver = firedrake_ts.DAESolver(
    problem, solver_parameters=params, monitor_callback=monitor
)
solver.solve()

print(f"\ndone on {Nfine}x{Nfine} mesh, Ra={Ra_val:.0e}, t_max={t_max:.0e}", flush=True)

if rank == 0: # in case of parallel run, only rank 0 will have the full history and do the plotting/saving
    print(f"final Nu = {Nu_history[-1]:.6f}", flush=True)

    np.savetxt(
        f"{outdir}/Nu_history_{Ra_str}_N{Nfine}.txt",
